...
> 以芥为舟，行万水千山。（当前版本 V2.6.5）

**芥舟**（V2.6）是一款以本地为数据主权的个人旅途工具，围绕出行三大主线：**行程规划 · 出行清单 · AA 记账**。默认所有数据只保存在你设备上（云端同步为可选项，需自行配置 Supabase 端点后显式登录才启用），无广告、无追踪。
...
- 云端同步 + 局域网离线兜底（移动端）：登录后跨设备双向同步，无网络时可用局域网同步多端汇总

### 🔐 隐私与数据
- 数据全部存储在本地 SQLite（Android）或浏览器 IndexedDB（Web）
- 备份/恢复：跨端交换使用 `.tat` / `.tav` 文件
- 本地为数据权威：云同步为可选能力（Supabase，可自托管/换镜像），未登录时单机完整可用
- 其他离线兜底：局域网同步、文件备份（.tat/.tav）
