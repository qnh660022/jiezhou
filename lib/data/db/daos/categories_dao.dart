...
class CategoriesDao extends DatabaseAccessor<AppDatabase> with _$CategoriesDaoMixin {
  ...
  Future<void> initBuiltin() async {
    ...
    for (final (k,n,i) in bs) { if (await getByKey(k)==null) { await into(categories).insert(CategoriesCompanion(key:Value(k),name:Value(n),icon:Value(i),builtin:Value(true))); SyncOutboxService.notifyWrite("categories", k); } }
  }
}
