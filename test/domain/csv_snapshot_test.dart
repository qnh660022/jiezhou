...
void main() {
  ...
  test('快照：BOM + CRLF + 转义 + 15 列逐字匹配', () {
    const expected = '\uFEFF'
        '日期,描述,分类,类型,金额元,币种,汇率,外币金额,付款人,分摊方式,分摊人数,备注,状态,关联行程,关联安排\r\n'
        '2025-08-25,东京塔登塔,ticket,正常,123.46,CNY,1.0000,,小王,平均,3,,未结,,\r\n'
        '2025-08-26,"晚餐,含""酒水""",餐饮,退款,-50.00,USD,7.2500,-6.90,小王,自定义,2,"第一行\n第二行",已结,,\r\n'
        '2025-08-27,酒店押金,stay,预付,900.00,CNY,1.0000,,小李、小王,自定义,2,押金待退,未结,关西之行,清水寺清晨\r\n';
    expect(csv, expected);
  });
  ...
  test('结构断言：BOM 开头、全 CRLF、含尾随空行', () {
    ...
    for (final line in lines.take(4)) {
      expect(line.split(',').length, greaterThanOrEqualTo(15),
          reason: '引号内逗号除外时列数应≥15');
    }
  });
